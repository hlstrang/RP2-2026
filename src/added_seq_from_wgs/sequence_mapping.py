...
def classify_and_validate(domains, expected_family):
    ...
    sp = [d for d in domains if d["name"] == "SP"]
...
if __name__ == "__main__":
    ...
    all_domains = sorted(
        hmm_domains + helix_domains + [signal_peptide],
    ...
    print("=" * 60)
    print("DUAL COLLAGEN VALIDATION REPORT")
    print("=" * 60)
    print("1. TARGETED VALIDATION (classify_and_validate):")
    print(f"   - Expected Target : {HCOL_TYPE.upper() if HCOL_TYPE else 'None'}")
    print(f"   - Identified As   : {results['known_identity']}")
    print(f"   - Status          : {results['validation_status']}")
    print("-" * 60)
    print("2. PROFILE MATRIX SCORES (classify_unknown_collagen):")
    print(f"   - Best Candidate  : {results['best_unknown_match']}")
    print(f"   - Confidence Level: {results['confidence'].upper()}")
    ...
    plot_title = (
        f"{TARGET_NAME.upper()} Architecture Map\n"
